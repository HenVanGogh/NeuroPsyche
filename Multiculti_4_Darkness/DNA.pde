...
*/
  ...
  float output(float[] previousLayer){
    ...
    for (int i = 0; i < previousLayer.length; i++) {
      result += (previousLayer[i] * weights[i]) + bias;
    }
    return result;
  }
  /*
...
class brain{
 ...
 void setNames(ArrayList<float[]> name){  
  ...
  void generateRandomStart(){
    for (neuron b: Sigmoid) {
     b.updateBias(random(-1 , 1));
     
     float[] randomTable = new float[input.length];
     for(int i = 0 ; i < input.length ; i++){
       randomTable[i] = random(-10  , 10);
     }
  ...
  void mutate(int mutationFactor){
    for (neuron b: Sigmoid) {
      b.mutateBias(random(-mutationFactor / 100 , mutationFactor / 100));
      for(int i = 0; i < Sigmoid.size() ; i++){
        b.mutateWeights(random(-mutationFactor / 100.0 , mutationFactor / 100.0) , i);
      }
    }
  ...
  void setObjective(float ox , float oy){
    objX = ox;
    objY = oy;
  }
