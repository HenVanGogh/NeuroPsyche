...
void mouseClicked() {
  shine b = new shine(mouseX , mouseY , 10);
    shines.add(b); 
}
...
if(Radiuso == 10){
  Radiuso = 100;
  print("R1");println(Radiuso);
}else{
 Radiuso = 10; 
 print("R2"); println(Radiuso);
}
}
...
void draw() {
  ...
  if (random(1) < 0.2) {
    float sz = random(4,8);
    particles.add(new Particle(random(width),random(height),sz));
  }
  }
